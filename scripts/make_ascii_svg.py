...
itserf in like a terminal, then holds.
...
USER = os.environ.get("GH_PROFILE_USER", "PardheevKrishna")
REAL_NAME = os.environ.get("GH_PROFILE_NAME", "Pardheev Krishna")
...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">{html.escape(USER)}@github: ~$ ./portrait.sh</text>')
...
status_text = f"{USER}@github:~$ whoami "
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'{html.escape(status_text)}<tspan fill="{INK}">{html.escape(REAL_NAME)}</tspan></text>')

cursor_x = PAD + (len(status_text) + len(REAL_NAME)) * 7.8
